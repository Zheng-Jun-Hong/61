    ...
    methods:{
        ...
        listen_svg(vm){
            ...
            document.querySelectorAll("g[data-hover]").forEach((obj) =>{
                obj.addEventListener("mouseover", function(e){
                    ...
                    hoverobj.children.forEach((element)=>{
            ...
            document.querySelectorAll("g[data-svg]").forEach((obj) => {
                ...
                obj.addEventListener("dblclick", function(){
                    window.location.href=`/stationGraphic?type=svg&plant=${that.plant_select[1]}&lgroup=${that.plant_select[2]}&svg=${attr}`
                })
            })
            //---------------------------------------------------------------------------------------------------------------
        ...
        toPvInfo(e) {
            ...
            this.axios.post("/equip_ID_get_group_data", {
                ...
                console.log(ID)
                if(that.$parent.$refs.stationPop !== undefined){
                    that.$parent.$refs.stationPop.$el.style.display = 'block';
                    that.$parent.$refs.stationPop.stationData = data.data.data
                    that.$parent.$refs.stationPop.jump_equip_ID = ID	
                    document.body.classList.add('lock')
                }else{
                    // If this SLD is already in the pop
                    // call function in stationPop
                    that.$parent.$emit('jump_in_pop', ID)
                }
            })
        }
    },
