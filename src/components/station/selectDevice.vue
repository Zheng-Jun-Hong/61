    ...
    <div id="selectDevice" class="selc_options_container" style="max-height: 55vh;">
        <div class="selc_options_wrap first_wrap">
            <div
                ...
                :key="idx"
            >
                {{ deviceType.name }}<i class="el-icon-arrow-right"></i>
            </div>
        </div>
        <div class="selc_options_wrap second_wrap" ref="secondWrap">
            <div
                ...
                :key="idx"
            >
                <i class="el-icon-check" v-show="device.isActive"></i>
                <span
                    class="equip_state"
...
export default {
    ...
    methods: {
        ...
        trans_data(){
            ...
            if(this.equipSelectData.ProtectiveRelay.length !== 0){
                let prelay_data = this.equipSelectData.ProtectiveRelay
                let prelay_total_list = []
                for(var i in prelay_data){
                    prelay_total_list.push({
                        name: prelay_data[i].name,
                        ID: prelay_data[i].ID,
                        isActive: false,
                        state: prelay_data[i].state,
                        type: "ProtectiveRelay",
                        collection: prelay_data[i].collection,
                    })
                }
                this.deviceList.push({
                    name: '保護電驛',
                    ID: '保護電驛',
                    children: prelay_total_list,
                    isActive: false,
                })
            }
            //跳轉至指定儀器
